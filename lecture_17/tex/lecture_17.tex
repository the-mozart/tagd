...
\frame[plain]{
	...
	\parbox{\textwidth}{
	Задача обтекания сферы вязкой жидкостью. Модель Стокса. Решение задачи обтекания сферы в рамках модели Стокса. Сравнение со случаем обтекания идеальной жидкости. Сила Стокса. Применимость теории Стокса. Формулы Озеена, Озеена -- Гольдштейна, Буссинеска.
	}
}
...
\frame{
	...
	\begin{exampleblock}{Постановка задачи (\textit{G.G Stokes}, 1851)}
		\parbox{\textwidth}{
			Определить силу, действующую на сферу радиуса $a$, движущуюся со скоростью $U$ в потоке вязкой жидкости, плотности $\rho$ и динамической вязкости $\mu$, при малых числах Рейнольдса 
			\[
...
\frame{
	...
	\begin{exampleblock}{Основные уравнения}
		...
		\frac{\ctg\theta}{r^2}\pd{v_r}{\theta} -\frac{2}{r^2} \pd{v_\theta}{\theta} - 
		\right.
		\]
		\[
		\left.
...
\frame{
	...
	\begin{exampleblock}{Компоненты тензора напряжений на поверхности сферы}
		\parbox{\textwidth}{
			\[
			\sigma_{rr}|_{r=a} = \left( -p + 2 \mu \pd{v_r}{r}\right)_{r=a} = \frac{3}{2}\mu\frac{U}{a}\cos\theta,
			\]		
			\[
...
\frame{
	...
	\centering
	\begin{tabular}{cc}
		\includegraphics[width=0.4\textwidth,frame]{../img/flow_sphere_Re_9_15.png} & 
		\includegraphics[width=0.4\textwidth,frame]{../img/flow_sphere_Re_17_9.png} \\	
		\includegraphics[width=0.4\textwidth,frame]{../img/flow_sphere_Re_25_5.png} & 
		\includegraphics[width=0.4\textwidth,frame]{../img/flow_sphere_Re_26_8.png} 
	\end{tabular}
...
\frame{
	...
	\centering
	\begin{tabular}{cc}
	\includegraphics[width=0.4\textwidth,frame]{../img/flow_sphere_Re_37_7.png} & 
	\includegraphics[width=0.4\textwidth,frame]{../img/flow_sphere_Re_73_6.png} \\	
	\includegraphics[width=0.4\textwidth,frame]{../img/flow_sphere_Re_118.png} & 
	\includegraphics[width=0.4\textwidth,frame]{../img/flow_sphere_Re_133.png} 
	\end{tabular}
...
%		\]
	...
	\begin{exampleblock}{Теория Озеена и Озеена -- Гольдштейна}
		\parbox{\textwidth}{
			\[
			c_z = \frac{24}{\Re}\left(
			...
			\right)
			\]
			Если сохранить только первый член, то будет формула Стокса, если первые два -- формула Озеена и т.д. 
			
			\medskip
			Формула Озеена получается при линеаризации конвективного слагаемого в уравнениях Навье -- Стокса по формуле
			\[
...
\frame{
	...
	\begin{exampleblock}{Формула Буссинеска}
		\parbox{\textwidth}{
			...
			\vspace{-5mm}			
			Сила для импульсно приведенного из состояния покоя в движение шара до скорости $U_0$:
			\[
