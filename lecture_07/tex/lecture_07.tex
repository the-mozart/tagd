...
\frame{
	...
	\begin{exampleblock}{Дифференциальная форма}
		\parbox{\textwidth}{
			\[
			\pd{\rho\vec{v}}{t} + \operatorname{div} (\rho\vec{v}\otimes\vec{v} + \sigma) = \rho\vec{f}.			
			\]
		}
	\end{exampleblock}
}
...
\frame{
	...
	\begin{exampleblock}{Интегральная форма}
		\parbox{\textwidth}{
			\[
			\frac{d}{dt}\int\limits_{\omega_t}\rho \vec{v}\times\vec{x} \, d\vec{x} = -\int\limits_{s_t}	\vec{\sigma}_n \times\vec{x} \, dS + \int\limits_{\omega_t}\rho \vec{f} \times\vec{x} \, d\vec{x},
			\]
			где $\rho\argtxv$, $\vec{v}\argtxv$ -- плотность и скорость материальной точки сплошной среды; $\vec{\sigma}_n\argtxv$ -- напряжение, возникающее на поверхности объема $\omega_t$, обозначенной $s_t$, на площадке с внешней единичной нормалью $\vec{n}$; $\vec{f}\argtxv$ -- массовая сила, действующая на сплошную среду.
		}
	\end{exampleblock}
