...
\frame[plain]{
	...
	\parbox{\textwidth}{
	Траектория движения сплошной среды. Формула Эйлера. Законы сохранения параметров сплошной среды в интегральной и дифференциальной форме. 
	}
}
...
%	=	\pd{v^i}{x^i}\Delta(t) = \Delta(t) \operatorname{div} \vec{v} 
	...
	\begin{exampleblock}{Правило дифференцирования определителя матрицы Якоби, или формула Эйлера}
		...
		\parbox{\textwidth}{
			...
			\right)=
			 \left| \pd{x^i}{\xi^j} \right|\operatorname{div}_x \vec{v}
			\]
		}
	\end{exampleblock}
...
\frame{
	...
	\begin{exampleblock}{Свойства}
		\parbox{\textwidth}{
		\medskip
		Пусть $f=f\argtxv$, $g=g\argtxv$, $\xv = \xv\argtxi$ тогда
			\[
				\frac{d}{dt}(f + g) = \frac{df}{dt} + \frac{df}{dt},\quad
				\frac{d}{dt}(f \cdot g) = \frac{df}{dt}\cdot g + f \cdot\frac{dg}{dt},
			\]
			\[
			\pd{f\argtxiv}{t} =  \frac{df\argtxv}{dt}.
			\]
		}
	\end{exampleblock}
...
\frame{
	...
	\begin{exampleblock}{Упрощения}
		...
		\[
		=
		\int\limits_{\omega_0} \left( \pd{F\argtxiv }{t} + F\argtxiv \operatorname{div}_{x}\vec{v}\argtxiv \right)\Delta\argtxiv  d\vec{\xi} =
		\]
		\[
		=
