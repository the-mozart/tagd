...
\frame{
	...
	\bigskip
	\parbox{\textwidth}{
		Рассмотрим напряжения, возникающие в точке $\vec{x}$ на площадке с единичной нормалью $\vec{n}$ и ей противоположной, то вследствие принципа равенства действия и противодействия получим:
		\[
...
\frame{
	...
	\begin{exampleblock}{}
		\parbox{\textwidth}{
		\small
		Пусть главные оси задаются ортонормированными векторами $\vec{g}_i$, а главные значения в этих осях тензора напряжения  $\sigma_i^j$ равны $\sigma_l$, тогда матрица перехода между ортонормированным базисом пространства $\vec{e}_j$ и введенным базисом будет ортогональная (обратная совпадает с транспонированной), т.е. 
		$
