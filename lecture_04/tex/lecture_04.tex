...
\frame[plain]{
	...
	\parbox{\textwidth}{
		Движение сплошной среды. Сопутствующий базис. Метрический тензор. Нелинейный тензор деформации. Геометрическая интерпретация компонент тензора деформаций. Главные деформации и инварианты. Связь между относительным изменением объёма и инвариантами тензора деформаций. 
	}
}
...
\frame{
	...
	\begin{exampleblock}{}
		\parbox{\textwidth}{
			Пусть задан закон деформирования тела в неподвижной фиксированной системе отсчёта
			$x^i = x^i\argtxi$, 	обладающий свойством гладкости и обратимости.
		}
	\end{exampleblock}
...
\frame{
	...
	\begin{exampleblock}{Определение}
		\parbox{\textwidth}{
			Координаты произвольной точки $P$ в абсолютной декартовой системе координат представляются в виде
			\[
...
\frame{
	...
	\begin{exampleblock}{Определение}
		\parbox{\textwidth}{
			...
			\]
			причём эти векторы зависят не только от координат точки $\argxi$, но и от времени $t$.
		}
	\end{exampleblock}
...
\frame{
	...
	\parbox{\textwidth}{
		...
		\] 
		а квадрат элемента дуги $ds_0$ равен
		\[
...
\frame{
	...
	\parbox{\textwidth}{
		Пусть при деформации точка $P_0$ перешла в точку $P$, а $P_0'$ в точку $P'$, тогда вектор $P_0P_0' = d\vec{r}_0$ перейдёт в вектор  $PP' = d\vec{r}$.
...
\frame{
	...
	\begin{exampleblock}{Определение}
		\parbox{\textwidth}{
			...
			По ранее доказанным теоремам $\varepsilon_{ij}$ -- тензорная величина и называется \alert{нелинейным тензором деформации}.
		}
...
\frame{
	\frametitle{ Эйлеров и лагранжев тензоры деформации }
	...
	\begin{exampleblock}{Определение}
		\parbox{\textwidth}{
			...
			\]
			В первом случае подход называется лагранжевым, а во втором эйлеровым. 
		}
	\end{exampleblock}
}
...
\frame{
	...
	\parbox{\textwidth}{
		...
		Однако две системы функций, вычисленных указанным путём, остаются различными, поэтому будем обозначать
		\[
...
\frame{
	...
	\begin{exampleblock}{Определение}
		\parbox{\textwidth}{
			Назовём \alert{удлинением $e$} изменение длины на единицу длины вектора $d\vec{r}_0=P_0P_0'$, так что
			\[
	...
	\begin{exampleblock}{Определение}
		\parbox{\textwidth}{
			\alert{Удлинения  $e_i$} в направлении базисных векторов $\vec{a}_i$ задаются формулами 
			\[
...
\frame{
	...
	\parbox{\textwidth}{
		...
		Если координаты начального состояния прямоугольные и декартовы, тогда $h_{ii}=1$. В случае малых деформаций, когда $2\varepsilon_{ii}/h_{ii} \ll 1$, 
		\[
			e_i \approx \varepsilon_{ii}.
		\]
		Таким образом величины $\varepsilon_{11}$, $\varepsilon_{22}$, $\varepsilon_{33}$ связаны с удлинением дуги, направленных вдоль базисных векторов $\vec{a}_1$, $\vec{a}_2$, $\vec{a}_3$.
	}
}
...
\frame{
	...
	\parbox{\textwidth}{
		Если рассмотреть случай, когда деформация происходит из состояния, когда система векторов $\vec{a}_i$ является ортонормированной, тогда $h_{ii}=1$, а $\theta_{ij}^0 = \pi/2$, если $i \neq j$. Пусть $\theta_{ij}=\pi/2-\alpha_{ij}$, тогда из полученных соотношений
		\[
...
\frame{
	...
	\]	\pause
	Таким образом, функции $\varepsilon_{ij}$ для $i \neq j$ указывают меру уменьшения первоначального прямого угла между элементами дуги, параллельными векторам $\vec{a}_i$ и $\vec{a}_j$.\pause
	
	\bigskip
	Компоненты $\varepsilon_{ij}$ для $i\neq j$ называются \alert{скалывающими (сдвиговыми)}  компонентами тензора деформации $E_0$. Компоненты $\varepsilon_{ii}$ -- \alert{нормальными} компонентами тензора $E_0$.
	}
	
}

\frame{
	\frametitle{ Геометрический смысл компонент тензора $E$  }
\parbox{\textwidth}{
...
\frame{
	...
	\parbox{\textwidth}{
			...
			\medskip
			Введём в рассмотрение квадратичную форму
			\[
			Q(\lambda) = \varepsilon_{ij}\lambda^i\lambda^j,
			\]
			и найдём максимальное значение этой квадратичной формы при
			\[
...
\frame{
	...
	Используя метод множителей Лагранжа задача сводится к отысканию решения
	\[
...
\frame{
	...
	В следствие симметричности тензора $\varepsilon_j^k$ эта система имеет три нетривиальных \alert{ортогональных решения} $\lambda^i_{(1)}$, $\lambda^i_{(2)}$, $\lambda^i_{(3)}$ $(i=1,2,3)$, отвечающих \alert{вещественным корням $\varepsilon_i$} кубического уравнения
	\[
	...
	\[
		...
		\end{array}
	\]
	$I_1$, $I_2$, $I_3$ -- \alert{инварианты} нелинейного тензора деформации $E$.
	}
}
...
\frame{
	...
	\parbox{\textwidth}{
	Таким образом существует ортонормированный базис, задаваемый векторами $\lambda^i_{(1)}$, $\lambda^i_{(2)}$, $\lambda^i_{(3)}$ $(i=1,2,3)$, в котором квадратичная форма принимает вид 
	\[
	Q(y) = \varepsilon_1(y^1)^2+\varepsilon_2(y^2)^2+\varepsilon_3(y^3)^2,
	\]
	а матрица тензора деформации $\varepsilon_{ij}$ становится диагональной
	\[
	\left\{
	\begin{array}{ccc}
	\varepsilon_1 & 0 & 0\\
	...
	Из геометрического смысла компонентов $\varepsilon_{ij}$ следует, что главные направлениями являются те ортогональные направления в недеформированном состоянии, которые остаются ортогональными после деформации.
...
\frame{
	...
	\begin{exampleblock}{Определение}
		\parbox{\textwidth}{
			Инварианты $I_1$, $I_2$, $I_3$ играют важную роль в построении моделей механики сплошной среды и выражаются через компоненты $\varepsilon_i^j$ следующим образом
			\[
...
\frame{
	...
	\parbox{\textwidth}{
		...
		\]
		в базисе собственный векторов $\lambda^i_{0(1)}$, $\lambda^i_{0(2)}$, $\lambda^i_{0(3)}$.
	}
...
\frame{
	...
	\parbox{\textwidth}{
		Из полученных соотношений удлинения вычисленные по начальным и конечным состояниям равны
		\[
...
\frame{
	...
	\begin{exampleblock}{Задача}
		\parbox{\textwidth}{
			Показать, что между инвариантами тензоров деформации $E$ и $E_0$ имеется следующая связь:
			\[
...
\frame{
	\frametitle{ Относительное изменение объёмов элементов }
	
	\parbox{\textwidth}{
		
	
	Из определения объёмного элемента следует, что 
	\[
...
\frame{
	...
	\parbox{\textwidth}{
		Рассмотрим метрические коэффициенты $h_{ij}$ как тензор в базисе $\vec{b}^j$, т.е. $H=h_{ij}\vec{b}^i\vec{b}^j$, определённых в пространстве переменных $\xi^i$ в конечном состоянии, так что
		\[
		...
		g|h_j^i| = h.
		\]
		Вследствие этого соотношение элементарных объёмов принимает вид
		\[
...
\frame{
	\frametitle{ Связь между изменением объёма и инвариантами тензора деформации }

	\parbox{\textwidth}{
		Из определения тензора деформации
		\[
		...
		\]\pause
		Таким образом, приближённо $\displaystyle\frac{d\tau-d\tau_0}{d\tau}=I_1$, а величину $I_1$ называют \alert{удельным расширением}.
	}
...
\frame{
	...
	\begin{itemize}[partopsep=1pt,label=\textbullet]
		\item Сокольников~И.~С. Тензорный анализ (теория и применение в геометрии и в механике сплошных сред).  Перевод с англ. Главная редакция физ.-мат. лит. Изд. М.: Наука, 1971.		
	\end{itemize}
