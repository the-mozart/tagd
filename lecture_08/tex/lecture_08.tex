...
\frame{
	...
	\parbox{\textwidth}{
		...
		\centering
		\normalsize
		$\lambda$, $\mu$ -- \alert{коэффициенты Ламе} или \alert{коэффициенты объёмной и динамической вязкости}.
...
\frame{
	...
	\begin{exampleblock}{Тензорная запись}
		\parbox{\textwidth}{
			...
			\]\pause
			\[
			\tau_{ij} =  (\lambda\delta_{ij} \delta_{km} + \mu (\delta_{ik} \delta_{jm} + \delta_{im} \delta_{jk})) e_{ij} = 
...
\frame{
	...
	\item {\em Басниев К. С., Дмитриев Н. М., Розенберг Г. Д.}. Нефтегазовая гидромеханика: Учебное пособие для вузов. -- М.-Ижевск:
