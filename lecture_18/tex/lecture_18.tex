...
\frame{
	...
	\begin{exampleblock}{Определение}
		\parbox{\textwidth}{
			Пусть определяющими переменными являются \alert{$\rho$} и \alert{$T$}, тогда
			\[
...
\frame{
	\frametitle{Термодинамический потенциал Гиббса}
		...
		\parbox{\textwidth}{
			...
			$H = \varepsilon -T S + pV $ называется \alert{потенциалом Гиббса}.
		}
	\end{exampleblock}
}
...
\frame{
	...
	\begin{exampleblock}{Свойства среды}
		\parbox{\textwidth}{
			Термодинамически и механические свойства идеальной двухпараметрической среды полностью определяются заданием одной из функций: $\varepsilon(V, S)$, $H(p, S)$, $F(V, T)$, $G(p, T)$.	Для пар переменных $p$ и $V$, $T$ и $S$ нет соответствующих потенциалов.
		}
	\end{exampleblock}
...
\frame{
	\frametitle{Удельные теплоёмкости $c_p$ и $c_V$}
	
	\begin{exampleblock}{Удельная теплоёмкость при постоянном давлении}
		\parbox{\textwidth}{
			\[
	...
	\begin{exampleblock}{Удельная теплоёмкость при постоянном объёме}
		\parbox{\textwidth}{
			\[
	...
	\begin{exampleblock}{Разность теплоёмкостей}
	\parbox{\textwidth}{
			\[
			c_p-c_V = 
			\left[
...
%
%\frame{
%	\frametitle{Разность теплоёмкостей}
%	
%	
%	
%}
...
\frame{
	...
	\begin{exampleblock}{Определение}
		\parbox{\textwidth}{
			\alert{Совершенным (идеальным) газом} называется газ, для которого справедлив закон Менделеева-Клайперона
			\[
			p V = R T,
			\]
...
\frame{
	\frametitle{ Совершенный газ }
		...
		\parbox{\textwidth}{
			Из полученных соотношений для внутренней энергии		
			\[
			\pd{\varepsilon}{V} = 0 \quad\Rightarrow\quad
			\varepsilon = \varepsilon(T),
			\]
			при этом удельная теплоёмкость $c_V= \displaystyle\pd{\varepsilon}{T} = c_V(T)$.
		}
	\end{exampleblock}\pause
...
\frame{
	...
	\begin{exampleblock}{Выводы кинетической теории}
		\parbox{\textwidth}{
			Выражения для удельных теплоёмкостей имеют вид
			\[
	...
	\begin{exampleblock}{Энтропия}
		\parbox{\textwidth}{
			Из соотношении для энтропии при заданных уравнениях состояния следует, что 
			\[
...
\frame{
	...
	\begin{exampleblock}{Уравнение состояния}
		\parbox{\textwidth}{
			Поправка к уравнению состояния идеального газа, связанная с учётом объёма молекул и сил молекулярного взаимодействия, приводит к уравнению состояния \alert{Ван-дер-Ваальса}
			\[
			p = \frac{RT}{V-b} - \frac{a}{V^2},
			\]
			где $a$ -- величина пропорциональная силе сцепления молекул, $b$ -- величина пропорциональная собственному объёму молекул газа.
...
\frame{
	...
	\begin{exampleblock}{Определение}
		\parbox{\textwidth}{
			...
			Если некоторый элемент газа подвергается медленному расширению или сжатию, так что при этом не происходит изменеия температуры газа, то такой процесс называется \alert{изотермическим}.
	...
	\begin{exampleblock}{Уравнение состояния идеального изотермического политропного газа}
		\parbox{\textwidth}{
			...
			\]
			В некоторых случаях идеальный изотермический газа можно рассматривать как политропный с показателем $\gamma=1$.
...
\frame{
	...
	\begin{exampleblock}{Определение}
		\parbox{\textwidth}{
			Газ называется \alert{нормальным}, если выполнены следующие свойства:
			...
			$
			\item Область переменных $(V,T)$, в которых выполнены свойства I-V -- выпуклая.
...
\frame{
	...
	\item 
	{\em Седов~Л.\,И.}  Механика сплошной среды. Том\,1. Гл.\,V, \S\,6.
