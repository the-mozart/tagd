...
\newcommand{\argxi}{(\xi_1,\xi_2,\xi_3)}
\newcommand{\argx}{(x_1,x_2,x_3)}

\newcommand{\argxbarn}{(\bar{x}^1,\bar{x}^2,\ldots, \bar{x}^n)}
\newcommand{\argxn}{(x^1, x^2,\ldots, x^n)}

\newcommand{\argtxi}{(t, \xi_1,\xi_2,\xi_3)}
\newcommand{\argtx}{(t, x_1,x_2,x_3)}
...
\frame{
	...
	\parbox{\textwidth}{
		Криволинейные системы координат. Скаляр. Вектор. Ковариантность и контравариантность. Тензор. Тензорная алгебра. Произведение тензоров. Сокращение индексов. Теоремы о тензорах. Фундаментальный квадратичная форма и тензор. Метрика. Скалярное произведение векторов.
	}
}
...
\frame{
	...
	\begin{exampleblock}{Определение}
		\parbox{\textwidth}{
			...
			Если для каждой системы координат $\argxn$ определена функция $f\argxn$, такая что при преобразования системы координат   (\ref{eq:coord_tranformation}) выполняется условие
			\[
			f\argxn = f\argxbarn,
			\]
...
\frame{
	...
	\[
	d\bar{x}^i = \pd{\bar{x}^i}{x^1}dx^1+\pd{\bar{x}^i}{x^2}dx^1+\ldots+
...
\frame{
	...
	\begin{exampleblock}{Определение}
	\parbox{\textwidth}{
		Если для каждой системы координат $x^\alpha$ определена совокупность $n$ функций $A_\alpha$ и если при преобразовании координат (\ref{eq:coord_tranformation}) эти функции преобразуются по формулам 
		\[
		\bar{A}_i = A_\alpha \pd{x^\alpha}{\bar{x}^i},
		\]
...
\frame{
	...
	\begin{exampleblock}{Определение}
		\parbox{\textwidth}{
			...
			\]
			то эти функции определяют \alert{контравариантный тензор второго ранга}, составляющим которого они являются.
...
\begin{exampleblock}{Определение}
	...
	\]
	то эти функции определяют \alert{ковариантный тензор второго ранга}, составляющим которого они являются.
...
\end{exampleblock}
	...
	\frame{
		...
		\begin{exampleblock}{Определение}
			...
			\]
			то эти функции определяют \alert{смешанный тензор второго ранга}, составляющим которого они являются.
 ...
 \frame{
 	...
 	\begin{exampleblock}{Символ Кронекера}
 		\[
 			...
 			\right.
 		\]
 		является смешанным тензором 2-ого ранга.
 	\end{exampleblock}
 ...
 \frame{
 	...
 	\begin{exampleblock}{Умножение тензора на число}
 		\parbox{\textwidth}{
 		При \alert{умножении} всех компонент тензора $A_{\alpha\beta}^{\gamma}$ \alert{на число} $\lambda$ получается новый тензор с компонентами $\lambda A_{\alpha\beta}^{\gamma}$ (док-во очевидно).
		} 		
 	\end{exampleblock}
 ...
 \frame{
	...
	\begin{exampleblock}{}
		\parbox{\textwidth}{
			По аналогии с ортогональными тензорами второго ранга можно показать, что любой тензор можно представить в виде суммы симметричного и антисимметричного по двум выбранным (одновременно ковариантным или контравариантным) индексам, причём единственным образом. 			
		}
 ...
 \frame{
 	...
 	\begin{exampleblock}{Определение}
 		...
 		Назовём произведением тензора $A_{\alpha}^\beta$ и тензора $B_{\gamma\delta}^\varepsilon$ тензор $C_{\alpha\gamma\delta}^{\beta\varepsilon}$, образуемый по формулам
 	...
 	\parbox{\textwidth}{
 		...
 		\]
 		тогда компоненты $C_{\alpha\gamma\delta}^{\beta\varepsilon}$ в новой системе координат
 		\[
 ...
 \frame{
 	...
 	Докажем, что тензор $B_\alpha = A_{\alpha\beta}^{\beta}$, полученный из смешанного тензора $A_{\alpha\beta}^{\gamma}$ путём суммирования по повторяющемуся индексу $\beta$ от $1$ до $n$, является тензором первого ранга.
	...
	\begin{exampleblock}{Вывод}
		\parbox{\textwidth}{
			Для каждого смешанного тензора можно можно получить новый тензор путём \alert{сокращения} одного ковариантного и контравариантного \alert{индекса}. 
		}
	\end{exampleblock} 	
 	}
 }
...
\frame{
 ...
 \frame{
 	...
 	\begin{exampleblock}{Теорема}
 		\parbox{\textwidth}{
 			Если для каждой системы координат $\argxn$ имеем совокупность $n^3$ величин $A_{\alpha\beta}^\gamma$ и если при любом выборе трёх векторов $u^\alpha$, $v^\beta$ и $w_\gamma$ выражение 
 			\[
 				f = A_{\alpha\beta}^\gamma u^\alpha v^\beta w_\gamma
 			\]
 			является инвариантом, то величины $A_{\alpha\beta}^\gamma$ являются составляющими тензора два раза ковариантного, раз ковариантного.
 		}
 	\end{exampleblock}
...
\frame{
	...
	\begin{exampleblock}{Доказательство}
		\parbox{\textwidth}{
			В силу произвольности $u^\alpha$, $v^\beta$ и $w_\gamma$ возьмём их так, что для наперёд заданных $i$, $j$, $k$ в новой системе координат  
			\[
			\bar{u}^\alpha = \delta^\alpha_i,\quad
			\bar{v}^\beta = \delta^\beta_k,\quad
			\bar{w}^\gamma = \delta_\gamma^j.
			\] \pause
			Тогда 
			$\displaystyle
			u^\alpha = \pd{x^\alpha}{\bar{x}^r}\bar{u}^r=\pd{x^\alpha}{\bar{x}^r}\delta^r_i=\pd{x^\alpha}{\bar{x}^i}$, $\displaystyle v^\beta = \pd{x^\beta}{\bar{x}^k}$, $\displaystyle w^\gamma = \pd{\bar{x}^j}{x^\gamma}$. \pause
			\[
				\bar{f} = \bar{A}_{\alpha\beta}^\gamma \bar{u}^\alpha \bar{v}^\beta \bar{w}^\gamma =  \bar{A}^j_{ik},\quad \pause
 ...
 \frame{
 	...
 	\begin{exampleblock}{Теорема}
 		\parbox{\textwidth}{
 			...
 			\[
 			B_{\alpha\beta} = \frac{1}{2}\left( A_{a\alpha\beta} + A_{\beta\alpha} \right)
 			\]
 ...
 \frame{
	 ...
	 \begin{exampleblock}{Доказательство}
	 	\parbox{\textwidth}{
			...
			\]
			инварианты по условию теоремы, а
			\[
...
\frame{
	\frametitle{ Свойства фундаментального тензора }
	...
	\begin{exampleblock}{Тензорная природа}
		\parbox{\textwidth}{
			\centering
...
\frame{
	...
	\begin{exampleblock}{Определение}
		\parbox{\textwidth}{
			Пусть $A^k$ -- произвольный фундаментальный тензор, тогда рассмотрим вектор
			\[
				A_i = g_{ik} A^k.
			\]
			\pause
			Эти равенства можно рассматривать как систему из $n$ линейных уравнений относительно $A^k$, и так как $g \neq 0$, то существуют такие числа $g^{ik}$, что
			\[
				A^i = g^{ik} A_k. 
			\] \pause 
			Т.к. $A_i$, $A_k$ произвольные векторы, то по признаку тензорного объекта $g^{ik}$ является контравариантным тензором и называется \alert{контравариантным фундаментальным тензором}.
	...
	\begin{exampleblock}{Свойства}
		\parbox{\textwidth}{
			\begin{itemize}
				...
				\]
				\item
				Сокращение смешанного фундаментального тензора по его индексам даёт размерность рассматриваемого многообразия, т.к. 
				\[
					g_i^i = \delta_i^i = n.
				\]
			\end{itemize}
...
\frame{
	\frametitle{ Переход между контравариантыми и ковариантным величинами}
	
	\begin{exampleblock}{Для векторов}
		\parbox{\textwidth}{
			\[
				A_i = g_{ik} A^k,\quad
				A^i = g^{ik} A_k, где			
	...
	\begin{exampleblock}{Для тензоров}
		\parbox{\textwidth}{
			\[
...
\frame{
	...
	Пусть подпространство $E_n$ вложено в евклидово пространство $E_m$ ($m \geq n$) и преобразование координат задаётся 
	\[\left\{
	\begin{array}{ccc}	
		y_1 & = & y_1\argxn,\\
		\vdots & \vdots & \vdots \\
...
\frame{
	...
	\parbox{\textwidth}{
	Рассмотрим котравариантный вектор $A^i$. Подберём бесконечно малый вектор $dx^i$, такой, что
	\[
...
\frame{
	...
	\[
		dy_\alpha =\pd{y_\alpha}{x^i} A^i \quad (\alpha=1,\ldots, m),
	\]
	тогда вектор $A^i=dx^i/\lambda$ в пространстве $R^m$ будет иметь компоненты 
	\[
		a_\alpha = \pd{y_\alpha}{x^i} A^i \quad (\alpha=1,\ldots, m).
	\]
	
	Рассмотрим ещё один вектор с компонентами $B^i$, имеющий в $R^m$ компоненты
	\[
...
\frame{
	...
	\begin{exampleblock}{Свойства}
		...
		\]	\pause
		\[
			cos\theta = \frac{	\vec{a}\cdot\vec{b} } {ab}= \frac{g_{ik}A^i B^k}{\sqrt{g_{ik} A^i A^k}\sqrt{g_{ik} B^i B^k}}
		\] \pause
		\[
